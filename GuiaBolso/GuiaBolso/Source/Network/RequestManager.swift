    ...
    func getRequest<T: Decodable>(
        ...
        service.dataTask(with: request as URLRequest) { (data, response, error) in
            ...
            if let serviceError = error {
                ...
                ErrorHandler().genericErrorHandling(title: "Error with service", message: error?.localizedDescription ?? "No Localized Description")
                return
            }
            
            guard let httpResponse =  response as? HTTPURLResponse else {
                completion(.failure(NotURLError(title: nil, description: "Couldn't use URL as Response")))
                ErrorHandler().genericErrorHandling(title: "Error with Http Response", message: error?.localizedDescription ?? "No Localized Description")
                return
            }
            ...
            if httpResponse.statusCode == 200 {
                ...
                guard let data = data else {
                    ...
                    ErrorHandler().genericErrorHandling(title: "Couldn't retrive data from URL", message: error?.localizedDescription ?? "No Localized Description")
                    return
                }
                ...
                do {
                    ...
                    ErrorHandler().genericErrorHandling(title: "Couldn't decode object retrivied from URL", message: error.localizedDescription )
                }
            } else {
